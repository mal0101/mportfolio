    ...
    <>
      <motion.header
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          isScrolled ? "bg-black/80 backdrop-blur-md border-b border-gray-800" : "bg-black/40"
        }`}
      >
        <nav aria-label="Main navigation" className="flex items-center justify-between px-6 py-4 my-0 md:px-12 md:py-5">
            ...
            {navLinks.map((link, index) => (
              <li key={link.label}>
                <button
                  ...
                  className="group relative font-mono text-xs tracking-wider text-gray-400 hover:text-white transition-colors duration-300"
                >
                  <span className="text-white/40 mr-1">0{index + 1}</span>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <span className="font-mono text-xs tracking-wider text-gray-400">AVAILABLE FOR WORK</span>
          </div>
          ...
          >
            <nav aria-label="Mobile navigation" className="flex flex-col items-center justify-center h-full gap-8">
              {navLinks.map((link, index) => (
                ...
                >
                  <span className="text-white/40 font-mono text-sm mr-2">0{index + 1}</span>
                  {link.label}
                </motion.button>
              ))}
              <motion.div
